...
<?php
	...
	<div id="product-grid">
		...
		<?php
		$product_array = $DB->query("SELECT * FROM product WHERE id_cat = 1");
		$product_array = $product_array->fetchAll();
		if (!empty($product_array)) { 
			foreach($product_array as $p){
				?>
				<div id="card_dr">
			<form method="post" action="dr.php?action=add&code=<?= $p['code'] ?>">
	...
	<div id="product-grid">
		...
		if (!empty($product_array)) { 
			...
			<div class="card" style="width: 18rem;">
				...
				<div class="card-body">
					<h4 class="card-title"><?= $p['nom_drone'] ?></h4>
					<div class="infos_drone">
					<p class="card-text">Prix : <?= $p['prix'] ?>€</p>
					<p class="card-text">Autonomie : <?= $p['autonomie'] ?>mn</p>
					<p class="card-text">Poids : <?= $p['poids'] ?>g</p>
					</div>
<!-- 					<div class="infos"> -->
				<input type="submit" value="Ajouter" class="btn" id="ajoute" width="100" />
                    <a href="afficher_produit.php?id=<?php echo $p['id_drone']; ?>" target="BLANK_" id="affiche"><input type="button" value="Description" class="btn"  /></a> 
